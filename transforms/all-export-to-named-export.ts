...
function getExportName(path: ASTPath<ExportNamedDeclaration>): string[] {
  ...
  if (declaration) {
    if (['ClassDeclaration', 'FunctionDeclaration', 'TSInterfaceDeclaration', 'TSTypeAliasDeclaration', 'TSEnumDeclaration'].indexOf(declaration.type) !== -1) {
      // @ts-ignore
...
const getExportedNames = (api: API, context: Context) => (
  baseDir: string,
  id: string
): ExportInfo => {
  ...
  if (!filePath) { // Not able to resolve ignore it
    return exportInfo;
  }
  ...
  try {
    ...
    sourceRoot.find(j.ExportAllDeclaration).forEach(path => {
      ...
      const exportedNames = getExportedNames(api, context)(Path.dirname(filePath), source);
...
const replaceAllExportForNamedExport = (api: API, filePath: string, context: Context) => (
  path: ASTPath<ExportAllDeclaration>,
  i: number
) => {
  const j = api.jscodeshift;
  const source = path.get("source", "value").value;
  ...
  exports = exports.concat(
    exportedNames.namedExports.map(name => {
